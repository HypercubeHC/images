...
void add(const std::vector<unsigned char>&file1, const std::vector<unsigned char>&file2, std::vector<unsigned char>&result){
    ...
    // передаём директивы OpenMP, указываем, что это будет выполняться параллельно,
    // приватная переменная для потока i, общие для всех file1 file2 result stop
    // выполняем в максимальном числе потоков, если пользователь не установил другое значение
